      ...
      if (app.id) {
        const rows = await sql`
          ...
          SET company=${company}, role=${role}, date=${date}, status=${status},
              ...
              next_action=${next_action}, follow_up_date=${follow_up_date}, updated_at=NOW()
          WHERE id=${app.id}
          RETURNING id, company, role, date, status, url, notes, source_lead_id, next_action, follow_up_date
        `;
        if (rows[0]) result.push(rows[0] as Application);
      } else {
        const rows = await sql`
          INSERT INTO applications (company,role,date,status,url,notes,source_lead_id,next_action,follow_up_date)
          VALUES (${company},${role},${date},${status},${url},${notes},${source_lead_id},${next_action},${follow_up_date})
          RETURNING id, company, role, date, status, url, notes, source_lead_id, next_action, follow_up_date
        `;
        if (rows[0]) result.push(rows[0] as Application);
      }
